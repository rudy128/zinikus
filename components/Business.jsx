  ...
  return (
    <>
        <section className='flex justify-center items-center mb-20'>
            <div className='w-1/2 space-y-10 ml-[5%] mr-24 relative py-24'>
        ...
        <section id='second-section' ref={secondSectionRef} className={`flex h-[500px] relative ${isSecondVisible ? 'animate-fadeIn' : 'opacity-0'}`}>
        <div className='space-y-10 ml-[5%] flex flex-col relative pb-24'>
          <h1 className={`text-7xl font-bold ${isSecondVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.5s]' : ''}`}>ZORO</h1>
          <h4 className={`font-bold text-2xl ${isSecondVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.6s]' : ''}`}>ZORO is an intelligent catering and food<br />delivery robot designed to revolutionize the<br />way  meals are served. Whether in<br />restaurants, hotels, or corporate spaces.</h4>
          <button className={`border border-black px-4 py-3 rounded-3xl text-lg font-bold w-fit ${isSecondVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.7s]' : ''}`}>Learn About Zoro</button>
        </div>
        <Image src={Zoro} alt='' width={500} height={720} className={`absolute right-20 z-10 ${isSecondVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.6s]' : ''}`} />
      </section>
        <section id='third-section' ref={thirdSectionRef} className={`h-[1087px] gap-x-10 relative flex justify-start p-10 pb-0 items-center bg-gradient-to-br from-[#27272d] to-85% to-[#8428e0] ${isThirdVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.5s]' : 'opacity-0'}`}>
            <div className='flex flex-col gap-y-40 w-1/2 ml-[3%]'>                
                <Image src={Zod} alt='' width={460} height={900} className={`${isThirdVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.6s]' : ''}`} />
            </div>
            <div className='w-1/2 mx-auto self-center text-white space-y-10'>
                <h1 className={`text-6xl font-bold ${isThirdVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.5s]' : ''}`}>ZOD</h1>
                <p className={`text-2xl ${isThirdVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.6s]' : ''}`}>Your brand deserves a representative that&apos;s smart, efficient,  and unforgettable. Meet ZOD, the cutting-edge robot  designed to elevate customer interactions with seamless  service and precision.</p>
                <button className={`text-lg font-bold border rounded-full px-4 py-2 ${isThirdVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.7s]' : ''}`}>Learn About ZORO</button>
            </div>
        </section>
        <section id='fourth-section' ref={fourthSectionRef} className={`flex h-[600px] relative ${isFourthVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.5s]' : 'opacity-0'}`}>
            <div className='gap-y-10 ml-[5%] relative flex flex-col py-12'>
                ...
                <h4 className={`font-bold text-2xl ${isFourthVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.6s]' : ''}`}>A smarter welcome begins with ZENO.<br />Designed to  redefine hospitality, ZENO is more<br />than just a robot &hyphen; it&apos;s an intelligent, interactive<br />guide that enhances guest experiences from<br />the moment they arrive.</h4>
                <button className={`bg-[#0f0e1733] px-4 py-3 w-fit rounded-3xl font-bold ${isFourthVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.7s]' : ''}`}>Learn About Zeno</button>
            </div>
            <Image src={Zeno} alt='' width={520} height={900} className={`absolute right-20 -top-1/4 ${isFourthVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.6s]' : ''}`}/>
        </section>
        <section 
          id='fifth-section' 
          ref={fifthSectionRef} 
        ...
        >
            ...
            <button className={`text-lg font-bold border rounded-full px-4 py-2 ${isFifthVisible ? 'inline-block opacity-0 animate-[appearFromBelow_1s_ease-out_forwards] [animation-delay:0.7s]' : ''}`}>See Atlas in Action</button>
        </section>
    </>
  )
}
